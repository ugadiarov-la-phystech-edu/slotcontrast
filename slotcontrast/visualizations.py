...
def create_grid_frame(frames, grid_size=(2, 6), image_size=(224, 224), padding=2):
    ...
    for index, frame in enumerate(frames):
        ...
        grid_frame[
            start_row : start_row + image_size[0], start_col : start_col + image_size[1]
        ] = frame
...
def mix_inputs_with_masks(inputs, outputs, softmasks=False):
    ...
    for t in range(masks_video.shape[0]):  # Iterate through each time step
        frames = [
            masks_video[t, i] for i in range(masks_video.shape[1])
        ]  # Get all frames for this time step
        ...
        video_frames = [
            video[0, :, t].permute(1, 2, 0).numpy() for i in range(masks_video.shape[1] + 1)
        ]
